...
async function run() {
  ...
  console.log('\n=== STEP 2: Deleting Project-Strix/Strix ===');
  try {
    const records = await pb.collection('curated_repos').getFullList({ filter: `full_name='Project-Strix/Strix'` });
    if (records.length > 0) {
      ...
      console.log('Project-Strix/Strix not found in database.');
    }
  } catch (err) {
